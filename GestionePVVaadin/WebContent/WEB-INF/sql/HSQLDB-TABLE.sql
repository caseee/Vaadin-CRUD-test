...
CREATE TABLE MOVIMENTATIONS (ID IDENTITY, OPDATE DATE, MOVIMENTATION VARCHAR(255), SITE INTEGER NOT NULL REFERENCES SITES ON DELETE CASCADE, MOVIMENTATION_TYPE INT NOT NULL REFERENCES MOVIMENTATION_TYPES ON DELETE CASCADE, INTERNALDESTINATION INT DEFAULT NULL REFERENCES SITES ON DELETE CASCADE, DESTINATION INT REFERENCES REGISTRY, VERSIONID INT DEFAULT 0  NOT NULL);

CREATE TABLE MOVIMENTATION_SPECS(ID IDENTITY, ID_HEAD INTEGER REFERENCES MOVIMENTATIONS ON DELETE CASCADE, ID_ART INTEGER REFERENCES ARTICLES, QUANTITY INTEGER, PRICE DECIMAL(10,2), VERSIONID INT DEFAULT 0 NOT NULL);
